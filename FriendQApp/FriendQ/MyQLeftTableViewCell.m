...
@implementation MyQLeftTableViewCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
		...
		self.userImage = [[[AsyncImageView alloc] initWithFrame:CGRectMake(10.0f, 10.0f, 120.0f, 120.0f)] autorelease];
...
-(void)setData:(Recommendation*)item {
    ...
    self.background.image = [UIImage imageNamed:@"myQBubbleLeft.png"];
...
-(void)layoutSubviews {
	...
	if(!self.editing) {
		...
		frame = CGRectMake(boundsX + 10, 10, 60, 60);
		self.userImage.frame = frame;
		
		frame = CGRectMake(boundsX + 110, 10, contentRect.size.width - 160, 20);
		self.thinkLabel.frame = frame;
		
		frame = CGRectMake(boundsX + 110, 30, contentRect.size.width - 160, 30);
		self.titleLabel.frame = frame;
		
		frame = CGRectMake(boundsX + 275, 25, 30, 30);
		self.iconView.frame = frame;
		
		frame = CGRectMake(boundsX + 79, 6, 234, 70);
